...
case 'lostpassword' :
	...
	if ( isset( $_REQUEST['error'] ) ) {
		if ( 'invalidkey' == $_REQUEST['error'] )
			$errors->add( 'invalidkey', __( 'Sorry, 该密钥无效。' ) );
		elseif ( 'expiredkey' == $_REQUEST['error'] )
			$errors->add( 'expiredkey', __( 'Sorry, 该密钥已经过期,重试.' ) );
	}
...
function retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) ) {
		$errors->add('empty_username', __('<strong>错误</strong>: 请输入用户名或邮箱地址.'));
	} else if ( strpos( $_POST['user_login'], '@' ) ) {
		$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
		if ( empty( $user_data ) )
			$errors->add('invalid_email', __('<strong>ERROR</strong>: 没有使用该电子邮件注册的用户.'));
	} else {
 ...
 $message = __('有人提交了重置下面账户密码的请求:') . "\r\n\r\n";
    $message .= '网站地址: ' .get_option('siteurl') . "\r\n\r\n";
...
$message .=  sprintf( '<p style="color: #6e6e6e;font-size:13px;line-height:24px;text-align:right;padding:0 32px">邮件来自：<a href="https://www.77nn.net" style="color:#51a0e3;text-decoration:none">讯沃blog</a></p>');
