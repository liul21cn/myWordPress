...
if ( is_single() ) {
global $post;
$copy1 = get_post_meta($post->ID, 'zzwz', true);
$copy2 = get_post_meta($post->ID, 'tgwz', true);
$copy = ($copy1) || ($copy2);
if (empty($copy)) {
...
<meta property="og:image" content="<?php echo get_post_img_url(); ?>" />
<meta property="og:release_date" content="<?php echo get_the_date('c');?>"/>
<?php
?>
<meta property="og:title" content="<?php echo get_the_title().strip_tags(get_the_subtitle()); ?> | <?php bloginfo('name');?>" />
<meta property="og:description" content="<?php echo tb_xzh_post_excerpt(); ?>" />
<?php
}else{
	}
}
...
function tb_xzh_post_imgs(){
  ...
  if(empty($src)){ //Defines a default image
   // $src = get_bloginfo('template_url')."/images/default.jpg";
	}
	return $src;
}
...
function tb_xzh_post_to_baidu() {
	if( _hui('xzh_on') && _hui('xzh_appid') && _hui('xzh_post_on') && _hui('xzh_post_token') ){
	    ...
	    if( 'publish' !== $post->post_status && $plink ){
			...
			update_post_meta($post->ID, 'Baidusubmit', $result_text);
		}
	}
}
?>